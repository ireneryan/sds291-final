...
Average Student Financial Status As A Predictor of Post-Graduation Earnings
Jessie Hinners, Natalie LaBossier, and Irene Ryan
...
There were several measures of tuition cost in the dataset, including average cost of attendance minus scholarships, average cost of public institutions, in and out of state tuition fees, among others, all of which were highly correlated. We chose average annual total cost of attendance, which includes tuition and fees, books and supplies, and living expenses for all full-time, first-time, degree/certificate-seeking undergraduates who receive Title IV aid. None of the other variables in our data set were significantly correlated. We tested the multicollinearity of our full model using the variance inflation factor in the car package. The only variable with questionable multicollinearity was percent of students with a Pell grant, with a VIF of 5.28. However, this is only slightly over the threshold of questionable multicollinearity. While this variable didn’t make it into our final model, we recognize that this could have affected the backward selection part of our stepwise regression.

There were three potentially high leverage outliers in the data. We tested them using a residuals vs leverage plot of our model. Because none of the the points are outside of Cook’s distance, none of our outliers have high leverage. 


##L.I.N.E. Conditions

The data’s linearity and equal variance conditions are met very well, as shown by the residuals vs. fitted plot. However, our Normality condition does not appear to be satisfied. This is most likely because our Median Family Income variable is not normally distributed, which makes sense because income is not normally distributed generally. To account for this, we ran a natural log transformation on the variable, which increased the normality of our model slightly, however the effect was minimal. The Independence condition is also questionable. While we initially assumed independence because most of these statistics are self-reported and federally required, we aren’t sure that the condition is actually met because there may be a relationship between schools in the same system/consortium or even within the same state.

#Results

Based on the p-values and coefficients of our model, average completion rate, average cost of attendance, median family income, and average SAT score are positively associated with mean earnings 10 years after graduation, while percent of students on Federal loans and average admission rate are negatively associated. The negative association between median family income, federal aid, and post graduation earnings indicates our hypothesis is confirmed: schools with a lower average financial status (lower family income, more federal aid) are correlated with lower earnings. This also indicates that more “prestigious” schools, i.e. schools with a lower admission rate, a higher average cost of attendance, and higher test scores are associated with higher earnings. On the other hand, schools with a lower median family income and more students receiving federal aid are associated with lower earnings. 
## Interaction Model
