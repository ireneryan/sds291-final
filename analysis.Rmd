...
collegescorecard%>%
  select(FAMINC, HI_INC_RPY_3YR_RT_SUPP)%>%
  cor()
...
```{r}
mod1<- lm(MN_EARN_WNE_P10~MD_FAMINC+SCH_DEG+ADM_RATE_ALL+C150_4+HIGHDEG+TUITFTE+TUITIONFEE_OUT+TUITIONFEE_IN+SAT_AVG+GRAD_DEBT_MDN_SUPP+COSTT4_A+PFTFAC+PCTPELL+NPT4_PUB+PCTFLOAN+RPY_3YR_RT_SUPP, data=collegescorecard)
summary(mod1)
```
```{r}
mod2<- lm(MN_EARN_WNE_P10~MD_FAMINC+SCH_DEG+ADM_RATE_ALL+C150_4+TUITFTE+TUITIONFEE_OUT+TUITIONFEE_IN+SAT_AVG+GRAD_DEBT_MDN_SUPP+COSTT4_A+PFTFAC+PCTPELL+NPT4_PUB+PCTFLOAN+RPY_3YR_RT_SUPP, data=collegescorecard)
summary(mod2)
```
```{r}
mod3<- lm(MN_EARN_WNE_P10~MD_FAMINC+ADM_RATE_ALL+C150_4+TUITFTE+TUITIONFEE_OUT+TUITIONFEE_IN+SAT_AVG+GRAD_DEBT_MDN_SUPP+COSTT4_A+PFTFAC+PCTPELL+NPT4_PUB+PCTFLOAN+RPY_3YR_RT_SUPP, data=collegescorecard)
summary(mod3)
```

```{r}
mod4<- lm(MN_EARN_WNE_P10~MD_FAMINC+ADM_RATE_ALL+TUITFTE+TUITIONFEE_OUT+TUITIONFEE_IN+SAT_AVG+GRAD_DEBT_MDN_SUPP+COSTT4_A+PFTFAC+PCTPELL+NPT4_PUB+PCTFLOAN+RPY_3YR_RT_SUPP, data=collegescorecard)
summary(mod4)
```

```{r}
mod5<- lm(MN_EARN_WNE_P10~MD_FAMINC+ADM_RATE_ALL+TUITFTE+TUITIONFEE_OUT+TUITIONFEE_IN+SAT_AVG+GRAD_DEBT_MDN_SUPP+COSTT4_A+PFTFAC+PCTPELL+PCTFLOAN+RPY_3YR_RT_SUPP, data=collegescorecard)
summary(mod5)
```
...
## Attempt a Correlation Matrix

```{r} 
library(car)
collegescorecard%>%
  select(MD_FAMINC, ADM_RATE_ALL,TUITFTE,TUITIONFEE_OUT,TUITIONFEE_IN,SAT_AVG,GRAD_DEBT_MDN_SUPP,COSTT4_A,PCTPELL,RPY_3YR_RT_SUPP)%>%
  cor()
```

Questions: 

cor() returning all NA?
...
stepwise to choose variables
pare down data to use a best subsets
Look at correlations
See what transformations we need to make in the data (EX: for debt repayment vs median earnings after 10 years, logistic?)
