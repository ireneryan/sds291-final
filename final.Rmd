...
```{r}
library(leaps)
full <- lm(MN_EARN_WNE_P10~MD_FAMINC+SCH_DEG+ADM_RATE_ALL+C150_4+HIGHDEG+SAT_AVG+GRAD_DEBT_MDN_SUPP+COSTT4_A+PFTFAC+PCTPELL+NPT4_PUB+PCTFLOAN+RPY_3YR_RT_SUPP data=nonadata)
...
model summary
```{r}
finalmod<-lm(formula = MN_EARN_WNE_P10 ~  HIGHDEG + ADM_RATE_ALL+
    SAT_AVG + GRAD_DEBT_MDN_SUPP + COSTT4_A + PCTPELL + NPT4_PUB + 
    PCTFLOAN + RPY_3YR_RT_SUPP+ MD_FAMINC, data = nonadata)
summary(finalmod)
